...
DB = os.environ.get("DB_PATH", "attendpro.db")
...
def init():  # create tables and demo data on first run
    with sqlite3.connect(DB) as c:
        ...
        CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY, name TEXT, username TEXT UNIQUE,
            password TEXT, role TEXT, balance INTEGER DEFAULT 20);
...
def dashboard():
    ...
    return render(PAGE, me=me, today=today, days=days, counts=[n.get(d, 0) for d in days],
...
def leave():
    ...
    if me:
        ...
        if days < 1 or days > me["balance"]:
            ...
            q("INSERT INTO leaves(user_id,kind,start_date,end_date,days,reason) VALUES(?,?,?,?,?,?)",
              me["id"], f["kind"], f["start"], f["end"], days, f["reason"])
...
<meta name=viewport content="width=device-width,initial-scale=1"><title>AttendPro</title>
...
<nav class="navbar navbar-dark mb-4"><div class=container>
 <span class="navbar-brand fw-semibold">AttendPro <small class="fw-normal opacity-75">Attendance and leave management</small></span>
...
<div class="row g-3 mb-3">
 <div class=col-md-4><div class="card h-100"><div class=card-body>
  ...
  <hr><h6>Leave balance</h6><div class=display-6>{{ me.balance }} days</div>
 </div></div></div>
...
<div class="card mb-3"><div class=card-body>
 ...
 <div class=table-responsive><table class="table table-sm mb-0">
  <tr><th>Name<th>Username<th>Role<th>Leave balance<th>Present today</tr>
